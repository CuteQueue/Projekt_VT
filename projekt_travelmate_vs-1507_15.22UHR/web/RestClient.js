...
// Model for Profils entity
    models.Profils = Backbone.Model.extend({
        urlRoot: "http://localhost:8080/projekt_travelmate_vs/api/users_profils.profils/",
        idAttribute: 'id',
        defaults: {
            mobilenumber: "",
            sex: "",
        ...
        sync: function (method, model, options) {
            ...
            if (method === 'create') {
                options.url = 'http://localhost:8080/projekt_travelmate_vs/api/users_profils.profils/';
            }
    ...
    // Collection class for Profils entities
    models.ProfilsCollection = Backbone.Collection.extend({
        model: models.Profils,
        url: "http://localhost:8080/projekt_travelmate_vs/api/users_profils.profils/",
        sync: function (method, model, options) {
            options || (options = {});
            var errorHandler = {
                error: function (jqXHR, textStatus, errorThrown) {
...
$(function () {
    ...
    var AppRouter = Backbone.Router.extend({
        ...
        list: function () {
            this.collection = new models.ProfilsCollection();
            var self = this;
            this.collection.fetch({
                success: function () {
                    self.listView = new views.ListView({
                        model: self.collection,
                        // tpl-profils-list-itemis template identifier for item
                        templateName: '#tpl-profils-list-item'
                    });
        ...
        details: function (id) {
            if (this.collection) {
                this.profils = this.collection.get(id);
                if (this.view) {
                    this.view.close();
                }
                var self = this;
                this.view = new views.ModelView({
                    model: this.profils,
                    // tpl-profils-details is template identifier for chosen model element
                    templateName: '#tpl-profils-details',
        ...
        create: function () {
            ...
            var self = this;
            var dataModel = new models.Profils();
            // see isNew() method implementation in the model
            dataModel.notSynced = true;
            this.view = new views.ModelView({
                model: dataModel,
                collection: this.collection,
                // tpl-profils-details is a template identifier for chosen model element
                templateName: '#tpl-profils-details',
