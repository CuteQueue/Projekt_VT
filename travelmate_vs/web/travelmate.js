    ...
    xmlhttp.onreadystatechange = function() {
        ...
        if(xmlhttp.readyState == 4 && xmlhttp.status == 200) {
            ...
            for(var i = 0; i < array.results.length;i++){
                var entry = array.results[i];
                $("input_field_"+i).value = entry.text;
                if(entry.belongs_to_caller == true){
                    document.getElementById("input_field_"+i).removeAttribute("readonly");
                    $("put_"+i).hidden = false;
                    $("del_"+i).hidden = false;
                }
